	...
	<view>
		...
		<view class="bodyone">
			<view class="body-child one">
				...
				<image src="../../static/searchOne.png" mode=""></image>
			</view>
...
<script>
	export default {
		...
		okOne(){
			uni.reLaunch({
				url:'./swipe-action?projectCode='+this.projectCode1+'&checkone='+this.checkone+''
			}) 	
		},
		 loginOr(){
			 if(!this.yes){
		 ...
		 ok(){
			 uni.reLaunch({
			 	url:'./swipe-action?projectCode='+this.projectCode2+'&checkone='+this.checktwo+''
			 }) 
		 },
		 anshi(){
			 uni.reLaunch({
